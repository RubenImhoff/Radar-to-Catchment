...
if FirstQues == 2:
	...
	if DataQuestion == "3":
		...
		dirpath = tkFileDialog.askdirectory(parent=root, title='Open the main folder with ASCII-files and press OK')
		list = subprocess.check_output(['find',dirpath,'-name','*.asc']).split('\n')[:-1]
		print(dirpath)
	...
	while True:
		...
		if StatQues == "1":
			...
			Resultpath = tkFileDialog.askdirectory(parent=root, title='Open the main folder with the resulting GeoTiff-files and press OK')
			ListResult = subprocess.check_output(['find',Resultpath,'-name','*.tiff']).split('\n')[:-1]
			...
			for x in range(0, len(ListResult)):
				...
				stats = srcband.ComputeStatistics(1)	
				print(stats)					
				writer.writerow([Basenametxt,stats[0],stats[1],stats[2],stats[3]])		
	...
	while True:
		...
		if Question == "1":
			...
			rasterfile = tkFileDialog.askopenfilename(parent = root, title='Choose a resulting file for visualisation')
